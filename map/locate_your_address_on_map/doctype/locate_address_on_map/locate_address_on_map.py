...
def get_latlon(address_form):
	...
	"""
	address_text= frappe.db.sql("select address_line1,ifnull(address_line2,' '), city ,ifnull(state,' '),ifnull(pincode,' '),country from tabAddress where name='{0}'".format(address_form),as_list=1)
	ret={'lat':'', 'lon':''}
	if address_text:
